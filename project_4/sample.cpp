...
// project 4
GLuint SpaceshipList; // Display list for spaceship
GLuint CubeList;	  // Display list for cube
GLuint CubeTexture;	  // Texture for cube walls
float WhiteMix = 0.2f;
int ObjectList;
float Eta;
float Mix;
...
void Display()
{
	...
	gluLookAt(0.f, 0.f, 15.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f);
	...
	TexturePattern.SetUniformVariable((char *)"uTexUnit", 0);
	glCallList(CubeList);
	TexturePattern.UnUse();
	...
	Pattern.SetUniformVariable("uWhiteMix", WhiteMix);
...
void InitGraphics()
{
	...
	TexturePattern.Init();
	bool textureValid = TexturePattern.Create((char *)"texture.vert", (char *)"texture.frag");
	if (!textureValid)
